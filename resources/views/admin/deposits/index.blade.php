        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div class="flex flex-wrap gap-2 text-sm">
                @foreach (['pending', 'approved', 'rejected', 'all'] as $tab)
                    <a href="{{ route('admin.deposits.index', ['status' => $tab]) }}"
    ...
    <div
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-black/5 dark:divide-white/10">
                    ...
                    <tr>
                        ...
                        <td class="px-5 py-4">
                            ...
                            @if ($tx->provider_reference)
                                <div class="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">ID:
                                    {{ $tx->provider_reference }}</div>
                            @endif
                            @if ($tx->receipt_path)
                                <a href="{{ \Illuminate\Support\Facades\Storage::url($tx->receipt_path) }}"
                                    target="_blank"
                                    class="mt-1 flex items-center gap-1 text-[10px] font-bold text-[#1877f2] hover:underline">
                        ...
                        <td class="px-5 py-4 text-right">
                            @if ($tx->status === 'pending')
                                <div class="flex justify-end gap-2">
                                    <form id="approve-form-{{ $tx->id }}" method="POST"
                                        action="{{ route('admin.deposits.decide', $tx) }}">
                                        @csrf
                                        <input type="hidden" name="action" value="approve" />
                                        <button type="button" data-confirm data-confirm-title="Approve Deposit"
                                            data-confirm-message="Are you sure you want to approve this deposit request?"
                                            data-confirm-text="Approve" data-confirm-type="success"
                                            data-confirm-on-confirm="#approve-form-{{ $tx->id }}"
                                            class="rounded-lg bg-blue-600 px-5 py-2.5 text-[10px] font-black uppercase tracking-widest text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700 transition">Approve</button>
                                    </form>
                                    <form id="reject-form-{{ $tx->id }}" method="POST"
                                        action="{{ route('admin.deposits.decide', $tx) }}">
                                        @csrf
                                        <input type="hidden" name="action" value="reject" />
                                        <button type="button" data-confirm data-confirm-title="Reject Deposit"
                                            data-confirm-message="Are you sure you want to reject this deposit request?"
                                            data-confirm-text="Reject" data-confirm-type="danger"
                                            data-confirm-on-confirm="#reject-form-{{ $tx->id }}"
                                            class="rounded-lg bg-rose-50 px-5 py-2.5 text-[10px] font-black uppercase tracking-widest text-rose-600 hover:bg-rose-100 transition">Reject</button>
                                    </form>
                                </div>
                            @else
                ...
                @empty
                    <tr>
                        <td class="px-5 py-8 text-sm text-zinc-600 dark:text-zinc-400" colspan="5">No deposit
