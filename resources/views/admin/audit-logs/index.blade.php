    ...
    <div class="rounded-lg bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-zinc-900 dark:ring-white/10">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <form method="GET" action="{{ route('admin.audit-logs.index') }}" class="w-full sm:w-80">
                ...
                <input id="q" name="q" value="{{ $search }}" placeholder="Search event…"
                    class="w-full rounded-full border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm outline-none focus:border-[#1877f2] focus:ring-2 focus:ring-[#1877f2]/15 dark:border-white/10 dark:bg-white/5" />
            </form>
        </div>
    </div>
                ...
                @forelse ($logs as $log)
                    <tr>
                        <td class="px-5 py-4 text-xs text-zinc-600 dark:text-zinc-400">
                            {{ $log->created_at?->toDateTimeString() }}</td>
                        <td class="px-5 py-4 font-semibold">{{ $log->event }}</td>
                        <td class="px-5 py-4 text-xs text-zinc-600 dark:text-zinc-400">{{ $log->actor?->email ?? '—' }}
                        </td>
                    </tr>
                @empty
                    <tr>
