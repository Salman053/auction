<x-admin-layout :title="'Shipping Rates'">
    <div class="rounded-lg bg-white p-5 shadow-sm ring-1 ring-black/5 dark:bg-zinc-900 dark:ring-white/10">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('admin.shipping_rates.create') }}"
                class="rounded-full bg-rose-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg transition hover:bg-rose-700">
                Add Shipping Rate
            </a>
        </div>
    </div>
    ...
    @if (session('status'))
        <div
            class="mt-4 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm font-bold text-emerald-600 dark:text-emerald-400">
            {{ session('status') }}
        </div>
    @endif
            ...
            <tbody class="divide-y divide-black/5 dark:divide-white/10">
                @foreach ($rates as $rate)
                    <tr>
                        ...
                        <td class="px-5 py-4 font-mono font-bold text-rose-600 dark:text-rose-400">
                            ¥{{ number_format($rate->fee_yen) }}</td>
                        <td class="px-5 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <a href="{{ route('admin.shipping_rates.edit', $rate) }}"
                                ...
                                <form id="delete-form-{{ $rate->id }}" method="POST"
                                    action="{{ route('admin.shipping_rates.destroy', $rate) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" data-confirm data-confirm-title="Confirm Deletion"
                                        data-confirm-message="Are you sure you want to delete this shipping rate for {{ $rate->name }}?"
                                        data-confirm-text="Delete" data-confirm-type="danger"
