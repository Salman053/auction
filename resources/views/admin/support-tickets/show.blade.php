<x-admin-layout :title="'Ticket #' . $ticket->id">
    ...
    <div class="mb-6">
        <div
            class="rounded-2xl bg-gradient-to-br from-white to-zinc-50/50 p-6 shadow-sm ring-1 ring-black/5 dark:from-zinc-900 dark:to-zinc-900/50 dark:ring-white/10">
                ...
                <div class="flex shrink-0 items-center gap-2">
                    @if ($ticket->status === 'open')
                        <form id="close-ticket-form" action="{{ route('admin.support-tickets.close', $ticket) }}"
                            ...
                            <button type="button" data-confirm data-confirm-title="Close Ticket"
                                ...
                                class="inline-flex items-center gap-2 rounded-xl bg-rose-50 px-4 py-2.5 text-sm font-semibold text-rose-600 shadow-sm transition-all hover:bg-rose-100 hover:shadow-md active:scale-[0.98] dark:bg-rose-900/20 dark:text-rose-400 dark:hover:bg-rose-900/30">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M6 18L18 6M6 6l12 12"></path>
                                </svg>
                    ...
                    @else
                        <form action="{{ route('admin.support-tickets.reopen', $ticket) }}" method="POST"
                            ...
                            @csrf
                            <button type="submit"
                                class="inline-flex items-center gap-2 rounded-xl bg-emerald-50 px-4 py-2.5 text-sm font-semibold text-emerald-600 shadow-sm transition-all hover:bg-emerald-100 hover:shadow-md active:scale-[0.98] dark:bg-emerald-900/20 dark:text-emerald-400 dark:hover:bg-emerald-900/30">
                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.support-tickets.index') }}"
                        class="inline-flex items-center gap-2 rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-semibold text-zinc-700 shadow-sm transition-all hover:bg-zinc-50 hover:shadow-md active:scale-[0.98] dark:border-white/10 dark:bg-white/5 dark:text-zinc-300 dark:hover:bg-white/10">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 flex flex-col gap-6">
            <div
                class="flex h-[calc(100vh-320px)] flex-col rounded-2xl bg-white shadow-sm ring-1 ring-black/5 dark:bg-zinc-900 dark:ring-white/10">
                <!-- Chat Header -->
        ...
        <div class="space-y-6">
            ...
            @else
                <div
                    class="rounded-2xl bg-gradient-to-br from-zinc-50 to-white p-7 text-center ring-1 ring-black/5 dark:from-white/5 dark:to-zinc-900 dark:ring-white/10">
                    ...
                    <form action="{{ route('admin.support-tickets.reopen', $ticket) }}" method="POST"
                        ...
                        @csrf
                        <button type="submit"
                            class="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-white px-6 py-2.5 text-sm font-semibold text-zinc-900 shadow-sm ring-1 ring-zinc-200 transition-all hover:bg-zinc-50 hover:shadow-md active:scale-[0.98] dark:bg-zinc-900 dark:text-white dark:ring-white/10 dark:hover:bg-zinc-800">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15">
                                </path>
            ...
            @if ($ticket->user)
                <div
                    ...
                    <div class="mb-5 flex items-center gap-3">
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-rose-500 to-rose-600 text-white shadow-md text-xl font-bold">
                    ...
                    <div class="mt-5">
                        <a href="{{ route('admin.users.index', ['search' => $ticket->user->email]) }}"
                            class="flex items-center justify-center gap-2 rounded-xl bg-slate-50 py-2.5 text-xs font-bold text-slate-700 transition-all hover:bg-slate-100 hover:shadow-sm dark:bg-white/5 dark:text-slate-300 dark:hover:bg-white/10">
