        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2 text-sm">
                @foreach (['open', 'closed', 'all'] as $tab)
                    <a href="{{ route('admin.support-tickets.index', ['status' => $tab]) }}"
        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-black/5 dark:divide-white/10">
                @forelse ($tickets as $ticket)
                    <tr>
                        <td class="px-6 py-4">
                            <a class="font-bold text-blue-600 hover:text-blue-700 transition"
                                href="{{ route('admin.support-tickets.show', $ticket) }}">{{ $ticket->subject }}</a>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            @if ($ticket->status === 'open')
                                <span
                                    class="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">OPEN</span>
                            @else
                                <span
                                    class="rounded-full bg-zinc-100 px-2 py-0.5 text-[10px] font-bold text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">CLOSED</span>
                            @endif
                        </td>
                        <td class="px-5 py-4 text-xs text-zinc-600 dark:text-zinc-400">
