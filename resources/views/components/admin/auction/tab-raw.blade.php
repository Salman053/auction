...
<div x-show="tab === 'raw'" class="mt-4 space-y-4">
            ...
            @if ($auction->raw && is_array($auction->raw))
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($auction->raw as $key => $value)
                        <div class="space-y-1">
                            <span class="text-[10px] font-black uppercase tracking-widest text-zinc-400 dark:text-zinc-500">{{ str_replace('_', ' ', $key) }}</span>
                            <div class="text-sm font-medium text-zinc-900 dark:text-white break-words">
                                @if (is_array($value))
                                    <pre class="mt-1 max-h-[200px] overflow-auto rounded-lg bg-zinc-50 p-2 text-[10px] font-mono text-zinc-600 dark:bg-black/20 dark:text-zinc-400">{{ json_encode($value, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                                @elseif (is_bool($value))
                                    <span class="rounded-full px-2 py-0.5 text-[10px] {{ $value ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700' }}">
                                        {{ $value ? 'TRUE' : 'FALSE' }}
                                    </span>
                                @else
                                    {{ $value ?: '—' }}
    ...
    @if ($auction->raw)
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-black/5 dark:bg-zinc-900 dark:ring-white/10">
            ...
            <div class="p-4">
                <pre class="max-h-[500px] overflow-auto rounded-xl bg-zinc-950 p-4 text-[10px] leading-relaxed text-emerald-400 font-mono dark:bg-black">{{ json_encode($auction->raw, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
            </div>
        </div>
    @endif
</div>
