@php
    $hasConsent =
        request()->cookie('cookie_consent') || (auth('user')->check() && auth('user')->user()->cookies_accepted);
@endphp

@if (!$hasConsent)
    <div x-data="cookieConsent()" x-show="show" x-cloak class="fixed inset-x-0 bottom-0 z-[100] p-4 sm:p-6 lg:p-8"
            ...
            <div
                ...
                <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex-1">
                        <div class="flex items-center gap-3">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-lg bg-brand-gold/10 text-brand-gold">
                    ...
                    <div class="flex shrink-0 flex-col gap-3 sm:flex-row">
                        <button @click="save(false)"
                            class="rounded-lg bg-slate-50 px-6 py-3 text-xs font-black uppercase tracking-widest text-slate-500 transition hover:bg-slate-100 dark:bg-white/5 dark:text-zinc-400 dark:hover:bg-white/10">
                            Reject All
                        </button>
                        <button @click="save(true)"
                            class="rounded-lg bg-brand-navy px-8 py-3 text-xs font-black uppercase tracking-widest text-brand-gold shadow-lg transition hover:scale-105 active:scale-95 dark:bg-brand-gold dark:text-brand-navy">
                            Accept All
                        </button>
                    </div>
                </div>
                ...
                <div x-show="showInfo" x-collapse class="mt-6 border-t border-slate-50 pt-6 dark:border-white/5">
                    <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            <h3 class="text-[10px] font-black uppercase tracking-widest text-brand-gold">Why we collect
                                it
                            </h3>
                            <ul class="mt-3 space-y-2 text-xs font-medium text-slate-500 dark:text-zinc-400">
                                <li class="flex items-start gap-2">
