...
<div
    {{ $attributes->merge(['class' => 'group relative flex flex-col overflow-hidden rounded-lg bg-white p-3 shadow-sm ring-1 ring-zinc-100 transition-all hover:shadow-2xl hover:ring-blue-500/20 dark:bg-zinc-900 dark:ring-white/5']) }}>
    <a href="{{ route('auctions.show', $auction) }}" class="flex-1">
        <div class="relative aspect-square overflow-hidden rounded-lg bg-zinc-50 dark:bg-zinc-800">
        ...
        <div class="mt-4 px-2">
            ...
            <h4
                class="mt-1 line-clamp-1 text-sm font-bold text-zinc-900 transition-colors group-hover:text-blue-600 dark:text-zinc-100">
                {{ $auction->title }}
            </h4>
            ...
            <div class="mt-6 flex items-center justify-between border-t border-zinc-50 pt-4 dark:border-white/5">
                ...
                <div class="text-right">
                    ...
                    <p
                        class="text-[10px] font-black text-zinc-600 dark:text-zinc-400 bg-zinc-100 dark:bg-white/5 rounded-md px-2 py-0.5">
                        {{ $auction->bid_count ?? 0 }} BIDS</p>
                </div>
            </div>
        </div>
    </a>
