<x-user-layout :title="$category->name">
    <div class="mb-12">
            ...
            <div>
                <nav class="mb-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-400">
                    ...
                    @if ($category->parent)
                        ...
                        <a href="{{ route('user.categories.show', $category->parent) }}" class="hover:text-blue-600">{{ $category->parent->name }}</a>
                    @endif
                </nav>
                <h1 class="text-4xl font-black tracking-tight text-zinc-900 dark:text-white uppercase tracking-tighter">
                    {{ $category->name }}
                </h1>
                <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Explore specific niches within the {{ $category->name }} sector.</p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ($category->children as $child)
                    <a href="{{ route('user.categories.show', $child) }}"
                        class="group flex items-center justify-between rounded-2xl bg-white p-6 shadow-sm ring-1 ring-zinc-200 transition hover:shadow-lg dark:bg-zinc-900 dark:ring-white/10">
                        <span class="text-sm font-black text-zinc-900 dark:text-white group-hover:text-blue-600 transition-colors">{{ $child->name }}</span>
        ...
        @else
            <div class="rounded-[2.5rem] bg-zinc-100 p-12 text-center dark:bg-white/5 border border-dashed border-zinc-300 dark:border-white/10">
                ...
                <p class="mt-2 text-zinc-500">No further sub-categories available. You can view all active listings in this category below.</p>
                <a href="{{ route('user.auctions.index', ['category' => $category->yahoo_category_id]) }}"
