    ...
    <div class="mb-12">
        <div class="mb-10">
            <h1 class="text-4xl font-black tracking-tight text-zinc-900 dark:text-white uppercase tracking-tighter">
                Market Explorer</h1>
            <p class="mt-2 text-sm text-zinc-500 dark:text-zinc-400">Browse thousands of niches across the Japan Auction
                network.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            @foreach ($categories as $category)
                <a href="{{ route('user.categories.show', $category) }}"
                    ...
                    <div
                        class="absolute -right-6 -top-6 h-24 w-24 rounded-full bg-zinc-50 dark:bg-white/5 transition-transform group-hover:scale-150">
                    </div>

                    <div class="relative z-10">
                        <div
                            class="mb-6 flex h-14 w-14 items-center justify-center rounded-lg bg-blue-600 text-white shadow-lg shadow-blue-600/20 group-hover:scale-110 transition-transform">
                            <span class="text-xl font-black">{{ substr($category->name, 0, 1) }}</span>
                        </div>

                        <h3
                            class="text-xl font-black tracking-tight text-zinc-900 dark:text-white group-hover:text-blue-600 transition-colors">
                            {{ $category->name }}
                        </h3>

                        <div
                            class="mt-4 flex items-center justify-between border-t border-zinc-100 pt-4 dark:border-white/5">
                            <span class="text-[10px] font-black uppercase tracking-widest text-zinc-400">View
                                Collection</span>
