...
function showDialog(detail = {}) {
    ...
    elements.icon.className = `flex h-14 w-14 shrink-0 items-center justify-center rounded-lg ${iconClasses(type)}`;
    elements.icon.innerHTML = iconSvg(type);

    elements.confirm.className = `rounded-lg px-8 py-3.5 text-sm font-black uppercase tracking-widest transition hover:scale-105 active:scale-95 shadow-xl ${confirmButtonClasses(type)}`;
    elements.confirm.dataset.onConfirm =
        typeof onConfirm === "string" ? onConfirm : "";
...
function openFromDataset(trigger) {
    ...
    if (amountSelector) {
        ...
        const amountRaw =
            amountInput && "value" in amountInput ? amountInput.value : null;
